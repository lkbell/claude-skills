---
name: tasks
description: Run Landon's day-to-day to-dos on his ClickUp kanban board (10 life-area Lists). Use whenever he wants to add, update, re-prioritize, or triage tasks — or whenever an action item surfaces in conversation that should be captured. Requires the ClickUp connector.
---

# Tasks — run Landon's ClickUp board

Landon's to-dos live on a ClickUp kanban board that **Claude operates and Landon supervises** — Claude does the hands-on work; Landon reviews and redirects on the board, and his own edits there take precedence.

Before working the board, **read the ✅ Tasks operating model** and follow it. It lives in **`tasks/README.md`** in the repo **lkbell/brain**, via the **GitHub connector**. The model holds what you can't guess: the Lists and how to route to them, the status set, task formatting, where tasks come from, the board-vs-docs split, the cadence, and the guardrails. (This skill points at `tasks/README.md` and hardcodes nothing else, so it survives brain restructuring.)

Two things worth holding up front: capture happens only in a live session (a scheduled job can't see the conversation), and you act within the Bible's guardrails — make reversible board changes freely, but confirm before anything that reaches a third party (e.g., a calendar invite to other people), spends money, or loses data.

Requires the ClickUp connector (the board itself) and the GitHub connector (`tasks/README.md` in `lkbell/brain`, for the operating model). If either isn't available on this surface, say so and proceed without it.
